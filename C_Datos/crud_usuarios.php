<?php
    ...
    {
        $sql = "SELECT id_usuario, usuario, clave, id_rol, estado 
                FROM usuario 
                WHERE usuario = :usuario AND estado = 1";
        
        error_log("Consulta SQL: $sql"); // Registra la consulta
        error_log("Buscando usuario: $usuario"); // Registra el usuario buscado
        
        $stmt = $this->conexion->prepare($sql);
        $stmt->bindValue(':usuario', $usuario); // IMPORTANTE: Dos puntos
        $stmt->execute();
        ...
        if ($result) {
            ...
            error_log("Usuario encontrado: " . print_r($result, true)); // Registra el usuario encontrado
            if (password_verify($contrasena, $result['clave'])) {
